        ...
        <section id="widget-grid" class="">
            <div class="row">
                ...
                <div class="jarviswidget" id="backpaltList"  data-widget-deletebutton="false" data-widget-editbutton="false">
                    ...
                    <!-- widget div-->
                    <div>
                        <form class="smart-form" id="Form" action="${contextPath}/trade/tradeInfo/list" method="post" >
                            <!-- widget edit box -->
                                ...
                                <div class="mt10 mb10">
                                    <table class="table lh32">
                                        ...
                                        <tr>
                                            ...
                                            <td nowrap="nowrap">
                                                <label class="input"  style="width:250px" >
                                                    <select class="select01" style="width:250px;" name="tradeSts" id="tradeSts">
                                                        <option <c:if test="${map.tradeSts == ''}"> selected="selected" </c:if> value="" >全选</option>
                                                        <option <c:if test="${map.tradeSts == '已自动入账'}"> selected="selected" </c:if> value="已自动入账" >已自动入账</option>
                                                        <option <c:if test="${map.tradeSts == '未入账'}"> selected="selected" </c:if> value="未入账" >未入账</option>
                                                        <option <c:if test="${map.tradeSts == '已手动退款'}"> selected="selected" </c:if> value="已手动退款" >已手动退款</option>
                                                        <option <c:if test="${map.tradeSts == '待手工处理'}"> selected="selected" </c:if> value="待手工处理" >待手工处理</option>
                                                    </select>
                                                </label>
                                            </td>
                                        </tr>
                            ...
                            <div>
                                <form class="smart-form" id="">
                                    ...
                                    <div class="widget-body-nobg-toolbar" style="overflow:hidden;">
                                        <button type="button" class="btn btn-default fl table-nobg-btn" id="tradeImport"><i class="fa fa-plus"></i>&nbsp;交易记录导入</button>&nbsp;
                                    </div>
                                        ...
                                        <table id="borrow-rep-table12" class="table table-bordered tc mt15" style="min-width:2800px;">
                                            ...
                                            <c:forEach items="${page.list}" var="t" varStatus="vs">
                                                <tr class="success">
                                                    ...
                                                    <td>${t.seqNo}</td>
                                                    <td>${t.chgCd}</td>
                                                    <td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${t.toAccTime}" /></td>
                                                    <td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${t.tradeTime}" /></td>
                                                    <td>${t.toAccNm}</td>
                                                    <td>${t.toAccNo}</td>
                                                    <td>${t.amount}</td>
                                                    <td>${t.tradeSts}</td>
