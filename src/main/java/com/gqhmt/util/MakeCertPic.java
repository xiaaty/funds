...
public class MakeCertPic {
	...
	public static String getCertPic(int width,int height,OutputStream os){  
		...
		//设定背景颜色  
		g.setColor(new Color(255,255,255));  //new Color(0xDCDCDC)
		g.fillRect(0,0,width,height);  
		//画边框  
//		g.setColor(Color.black);  g.drawRect(0,0,width-1,height-1);  
		//随机产生的验证码  
		String strEnsure = "";  
		//4代表4位验证码，如果要产生更多位的验证码，则加大数值 
		for(int i = 0;i<4;++i){   
			strEnsure += mapTable[(int) (mapTable.length*Math.random())];  
		}  
		//将认证码显示到图像中，如果要生成更多位的验证码，增加drawString语句  
		g.setColor(new Color(57,72,40));  
		g.setFont(new Font("Atlantic Inline",Font.PLAIN,22));  
		String str = strEnsure.substring(0,1); 
		g.drawString(str,15,20);  
		str = strEnsure.substring(1,2);  
		g.drawString(str, 30, 20); 
		str = strEnsure.substring(2,3);  
		g.drawString(str, 45, 20);  
		str = strEnsure.substring(3,4);  
		g.drawString(str, 60, 20);  
		//随机产生15个干扰点  
